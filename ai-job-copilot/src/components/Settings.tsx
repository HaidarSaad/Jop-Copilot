...
export default function Settings({ language, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-lg w-full p-6" dir={language === "ar" ? "rtl" : "ltr"} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-5">
            ...
            <div className="grid gap-2">
              {options.map((o) => (
                <button key={o.id} onClick={() => setProvider(o.id)}
                  className={`flex items-center gap-3 p-3 rounded-xl border-2 text-left transition-all ${provider === o.id ? "border-blue-500 bg-blue-50 dark:bg-blue-900/30" : "border-slate-200 dark:border-slate-600 hover:border-slate-300 dark:hover:border-slate-500"}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${provider === o.id ? "border-blue-500" : "border-slate-300"}`}>
                    {provider === o.id && <div className="w-2 h-2 rounded-full bg-blue-500" />}
                  </div>
                  <div>
          ...
          <div>
            ...
            <input type="text" value={apiKey}
              ...
              className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-white dark:bg-slate-700 text-slate-800 dark:text-white"
              dir="ltr"
            />
          ...
          <div className="flex gap-3">
            <button onClick={handleSave}
              className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all font-medium text-sm">
              {saved ? "✓ " + loc("تم الحفظ", "Saved") : loc("حفظ", "Save")}
            </button>
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-700/30 rounded-xl text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            <p className="font-medium mb-1">🔒 {loc("الخصوصية والأمان", "Privacy & Security")}</p>
